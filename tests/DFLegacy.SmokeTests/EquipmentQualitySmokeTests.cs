...
internal static class EquipmentQualitySmokeTests
{
    ...
    {
        const ulong hardwareSeedSample = ulong.MaxValue;
        var hardwareSampleAvailable = true;
        bool TryGetHardwareSeed(out ulong value)
        {
            value = hardwareSeedSample;
            if (!hardwareSampleAvailable)
            {
                return false;
            }
        ...
        var hardwareRandom = new GameRandomSource(
            cpuSupportsRdSeed: true,
            nativeBridgeLoaded: true,
            rdSeedEnabled: true,
            TryGetHardwareSeed,
            _ => throw new InvalidOperationException(
                "OS fallback must not be used for the quality seed"),
            _ => throw new InvalidOperationException(
                "PRNG fallback must not be used for the quality seed"));
        var hardwareRoll = EquipmentQuality.RollSeed(hardwareRandom);
        var expectedHardwareRoll = checked(
            1u + (uint)(hardwareSeedSample
                % EquipmentQuality.MaximumRandomSeed));
        ...
        check(hardwareRoll == expectedHardwareRoll
                && hardwareRandom.GetStatus() is
                {
                    RdSeedSamples: 1,
            ...
            "equipment quality consumes the hardware-seed path, uses DF60A1's 1..999999997 range, and retains the deterministic changed-value fallback");
